	...
	while(TRUE)
	{
		...
		if(TRUE == bRet)
		{
			...
			case FILE_ACTION_ADDED:
				{
					//TRACE("添加: %s\r\n", szFileName);
					Add_Time = str;
					p_showlistctrl->InsertItem(p_showlistctrl->GetItemCount(),str + tp1->szbuf + L"添加:"+(CString)szFileName);
					break;
				}
			case FILE_ACTION_REMOVED:
				{
					//TRACE("删除:%s\r\n", szFileName);
					p_showlistctrl->InsertItem(p_showlistctrl->GetItemCount(),str + tp1->szbuf + L"删除:"+(CString)szFileName);
					break;
				}
			case FILE_ACTION_MODIFIED:
				{
					...
					if(Add_Time == Modefi_Time)
						break;
					p_showlistctrl->InsertItem(p_showlistctrl->GetItemCount(),str + tp1->szbuf + L"修改:"+(CString)szFileName);
					break;
				}
						...
						case FILE_ACTION_RENAMED_NEW_NAME:
							{
								...
								::WideCharToMultiByte(CP_ACP,
									...
									NULL);
								//TRACE(" -> %s\r\n", szFileName);
								p_showlistctrl->InsertItem(p_showlistctrl->GetItemCount(),str + tp1->szbuf + L"重命名:"+old_name+ L"->"+(CString)szFileName);
								break;
							}
						}
					}
			...
			case FILE_ACTION_RENAMED_NEW_NAME:
				{
					//TRACE("重命名(new):%s\r\n", szFileName);
					p_showlistctrl->InsertItem(p_showlistctrl->GetItemCount(),str + tp1->szbuf + L"重命名(new):"+(CString)szFileName);
					break;
				}
			}         
		}
